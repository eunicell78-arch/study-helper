...
# Generic meta-words that alone are not useful as search queries.
# These are configurable: add/remove as needed.
GENERIC_QUERY_TERMS = {
    "수행평가", "보고서", "자료조사", "분석", "서술", "정리",
    "조사", "탐구", "발표", "설명", "논술", "작성",
}


def _normalize_query(q: str) -> str:
    """Casefold, strip, and remove all punctuation/spaces for comparison."""
    q = q.strip().casefold()
    q = re.sub(r'[\s\W]+', '', q, flags=re.UNICODE)
    return q


def post_process_search_queries_ko(
    search_queries_ko: list,
    subject: str,
    topic: str,
    element_verbatim: str,
) -> list:
    """Post-process LLM-generated Korean search queries (Policy 2).

    Steps:
    1. Normalise and deduplicate.
    2. Remove queries that are essentially equal to subject or topic alone
       (after stripping punctuation/spaces/casing).
    3. Remove queries that consist only of generic meta-words.
    4. Remove queries shorter than 2 characters after normalisation.
    5. Prefer queries that contain element-specific keywords.
    6. If fewer than 6 remain, generate additional queries from element_verbatim
       keywords combined with topic (and subject when available).
    7. Keep the best 6–10 (prefer longer/more specific and element-keyword-
       containing queries).
    """
    norm_subject = _normalize_query(subject)
    norm_topic = _normalize_query(topic)

    # Build a set of "too-generic" normalised strings to reject
    reject_norms: set[str] = set()
    if norm_subject:
        reject_norms.add(norm_subject)
    if norm_topic:
        reject_norms.add(norm_topic)
    # Also reject combined "subject+topic" without anything else
    if norm_subject and norm_topic:
        reject_norms.add(norm_subject + norm_topic)
        reject_norms.add(norm_topic + norm_subject)

    kept: list[str] = []
    seen_norms: set[str] = set()

    for q in search_queries_ko:
        ...
        if len(norm_q) < 2:
            continue
        # Skip duplicates
        if norm_q in seen_norms:
            continue
        seen_norms.add(norm_q)
        # Skip if equal to subject/topic (or combined) alone
        if norm_q in reject_norms:
            continue
        # Skip if the query is only generic meta-words (Korean only; keep English/mixed)
        words_in_q = set(re.findall(r'[가-힣]+', q))
        if words_in_q and words_in_q.issubset(GENERIC_QUERY_TERMS):
            continue
        kept.append(q)

    # Extract element-specific keywords for gap-filling and scoring
    # Use build_search_query logic: strip evaluation endings and extract nouns
    element_text = element_verbatim
    element_text = re.sub(
        r'(했는가|하였는가|서술하였는가|제시하였는가|있는가|했나|하였나|'
        r'되었는가|되었나|였는가|인가|였나|하는가|하였나|포함하였는가|'
        r'활용하였는가|분석하였는가|설명하였는가|논하였는가)[?？.]*',
    ...
    element_text = re.sub(r'\d+\s*(개|가지|건|자|줄|문장)\s*(이상|이내|이하|내외)', '', element_text)
    element_text = re.sub(r'[,;·:]+', ' ', element_text)
    # Extract Korean noun-like tokens (2–6 chars) not equal to subject/topic
    element_tokens = [
        t for t in re.findall(r'[가-힣]{2,6}', element_text)
        if _normalize_query(t) not in reject_norms
        and t not in GENERIC_QUERY_TERMS
    ]
    ...
    # Score kept queries: prefer those containing element tokens and longer ones
    def _score(q: str) -> tuple:
        has_element_kw = any(t in q for t in unique_tokens)
        return (int(has_element_kw), len(q))

    kept.sort(key=_score, reverse=True)

    # Fill up to 6 if needed
    if len(kept) < 6:
        base = topic.strip() if topic.strip() else subject.strip()
        for token in unique_tokens:
            if len(kept) >= 6:
                break
            candidate = f"{base} {token}".strip() if base else token
            norm_c = _normalize_query(candidate)
            if norm_c not in seen_norms and norm_c not in reject_norms:
                seen_norms.add(norm_c)
                kept.append(candidate)
        # If still short, generate topic+element combos more aggressively
        if len(kept) < 6 and base and unique_tokens:
            for i in range(0, len(unique_tokens) - 1):
                if len(kept) >= 6:
                    break
                candidate = f"{base} {unique_tokens[i]} {unique_tokens[i+1]}".strip()
                norm_c = _normalize_query(candidate)
                if norm_c not in seen_norms and norm_c not in reject_norms:
                    seen_norms.add(norm_c)
                    kept.append(candidate)
        # Last resort: use the full build_search_query result
        if len(kept) < 6:
            fallback = build_search_query(subject, topic, element_verbatim)
            norm_fb = _normalize_query(fallback)
            if norm_fb not in seen_norms and norm_fb not in reject_norms:
                seen_norms.add(norm_fb)
                kept.append(fallback)

    # Cap at 10 (best scored first, already sorted)
    return kept[:10]
...
def make_a_layer_sources(
    ...
    Prefers search_queries_ko (top 2 items) when available so that search
    result pages use the LLM-recommended Korean keywords and have URL diversity.
    Falls back to build_search_query(subject, topic, element_verbatim) when
    search_queries_ko is empty or not provided.

    Returns one candidate per template in SEARCH_TEMPLATES.  The first two
    templates use query1 and query2 respectively (if available); remaining
    templates reuse query1 to stay deterministic.
    """
    if search_queries_ko:
        query1 = search_queries_ko[0]
        query2 = search_queries_ko[1] if len(search_queries_ko) > 1 else query1
    else:
        query1 = build_search_query(subject, topic, element_verbatim)
        query2 = query1
    short = element_verbatim[:30]
    suffix = "…" if len(element_verbatim) > 30 else ""
    templates = list(SEARCH_TEMPLATES)
    sources = []
    for idx, (site_name, template) in enumerate(templates):
...
def make_research_guide_for_element(
    ...
    try:
        ...
        guide["search_queries_ko"] = post_process_search_queries_ko(
            guide["search_queries_ko"], subject, topic, element_verbatim
        )
        return guide
    except Exception:
        fallback_queries = post_process_search_queries_ko(
            [], subject, topic, element_verbatim
        )
        return {
            ...
            "search_queries_ko": fallback_queries,
            "search_queries_en": [],
        }
